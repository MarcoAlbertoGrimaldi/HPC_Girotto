...
int main( int argc, char * argv[] ){
    ...
    for(int i=0; i<npes; ++i){
        ...
        MPI_Recv(&temp, N, MPI_INT, ((rank-1)%npes), ((rank-1)%npes), MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
/*  for(int i=0; i<npes; ++i){
        ...
        MPI_Irecv(&temp, N, MPI_INT, ((rank-1)%npes), ((rank-1)%npes), MPI_COMM_WORLD, &request);
        MPI_Barrier(MPI_COMM_WORLD);
